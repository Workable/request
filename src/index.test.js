...
describe("authClient", () => {
  it("requests token if authClient is provided", async () => {
    const clearMock = mockResponseOnce();
    const authClient = {
      getTokenSilently: jest
        .fn()
        .mockReturnValue(Promise.resolve({ id_token: "myToken" }))
    };
    const response = await request("mockUrl", {
      authClient,
      withAuthClientError: jest.fn().mockImplementation(n => n)
    });
    expect(authClient.getTokenSilently).toBeCalled();
    expect(response).toBe("12345");
  ...
  it("handles authClient unexpected error", async () => {
    // we will update the test when we fix the redirection
    let error;
    const authClient = {
      getTokenSilently: jest.fn().mockReturnValue(Promise.reject("myError"))
    };
    try {
  ...
  it("handles authClient normal error", async () => {
    // we will update the test when we fix the redirection
    let error;
    const authClient = {
      getTokenSilently: jest
        .fn()
        .mockReturnValue(Promise.resolve({ error: { name: "AuthError" } }))
    };
    try {
      await request("mockUrl", {
        authClient,
        withAuthClientError: jest.fn().mockImplementation(n =>
          n.then(response => {
            if (response.error.name === "AuthError") throw "myError";
          })
        )
      });
    } catch (e) {
  ...
  it("doesn't requests token if authClient is provided", async () => {
    const clearMock = mockResponseOnce();
    const response = await request("mockUrl", {});
    expect(response).toBe("12345");
